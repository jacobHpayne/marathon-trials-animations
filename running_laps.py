...
class Runner
    ...
    def __init__(self, canvas, xy, ink, speed, mode)
        ...
        # the scale speed is based on the ratio below. Using 1006 comes from the scale distance of 100 pixels per mile
        ...
        self.label = self.canvas.create_text(xy[0], xy[1], text=     +str(self.laps))  # spaces for legibility
    ...
    def run(self)
        ...
        if self.mode == 'circuit'
            ...
            if self.theta  1.55 and not self.toggle
                ...
                self.canvas.itemconfigure(self.label, text =     +str(self.laps))
...
# Change runner start coordinates, color, speed (minmile) and start wave (delay set above, currently 10 scale minutes)
